...
def modify_keymap():
    ...
    for keymap_path in keymap_paths:
        ...
        def has_play_trailer_tag(tag):
            return tag.text == "RunScript(script.NXTastic.helper, mode=play_trailer)"
        ...
        if setting_value:
            if t_key_tags:
                t_key_tags[
                    0
                ].text = "RunScript(script.NXTastic.helper, mode=play_trailer)"
                for tag in t_key_tags[1:]:
                    keyboard_tag.remove(tag)
            else:
                t_key_tag = ET.SubElement(keyboard_tag, "t")
                t_key_tag.text = "RunScript(script.NXTastic.helper, mode=play_trailer)"
            if play_pause_tags:
                play_pause_tags[
                    0
                ].text = "RunScript(script.NXTastic.helper, mode=play_trailer)"
                ...
                play_pause_tag.text = (
                    "RunScript(script.NXTastic.helper, mode=play_trailer)"
                )
        else:
