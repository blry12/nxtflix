...
class source:
	def __init__(self, scrape_provider, scraper_name, folder_path):
		...
		self.assignment_test_string = 'NXTFlix_FOLDERS_%s_%s' % (self.scrape_provider, '%s')
	...
	def results(self, info):
		try:
			...
			from modules.kodi_utils import logger
			logger('NXTFlix folders scraper Exception', str(e))
		internal_results(self.scraper_name, self.sources)
		return self.sources

	def _gather_assigned_content(self):
		pre_assigned_content = gather_assigned_content("'NXTFlix_FOLDERS_%s" % self.scrape_provider + "_%'")
