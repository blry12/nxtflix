...
download_directories_dict = {'movie': 'NXTFlix.movie_download_directory', 'episode': 'NXTFlix.tvshow_download_directory', 'thumb_url': 'NXTFlix.image_download_directory',
							'image_url': 'NXTFlix.image_download_directory','image': 'NXTFlix.image_download_directory', 'premium': 'NXTFlix.premium_download_directory',
							None: 'NXTFlix.premium_download_directory', 'None': False}
...
paginate_dict = {True: 'NXTFlix.paginate.limit_widgets', False: 'NXTFlix.paginate.limit_addon'}
prescrape_scrapers_tuple = ('furk', 'easynews', 'rd_cloud', 'pm_cloud', 'ad_cloud', 'folders')
sort_to_top_dict = {'folders': 'NXTFlix.results.sort_folders_first', 'rd_cloud': 'NXTFlix.results.sort_rdcloud_first',
					'pm_cloud': 'NXTFlix.results.sort_pmcloud_first', 'ad_cloud': 'NXTFlix.results.sort_adcloud_first'}
...
def generic_list_sorting(setting_id):
	return get_setting('NXTFlix.sort.%s' % setting_id, 'popularity.desc').replace('&amp;', '&')
...
def use_skin_fonts():
	return get_setting('NXTFlix.use_skin_fonts')

def use_custom_skins():
	return get_setting('NXTFlix.use_custom_skins', 'true') == 'true'

def results_format():
	return str(get_setting('NXTFlix.results.list_format', 'List').lower())

def results_style():
	return results_style_dict[get_setting('NXTFlix.results.use_contrast', 'true')]
...
def store_resolved_torrent_to_cloud(debrid_service, pack):
	setting_value = int(get_setting('NXTFlix.store_resolved_torrent.%s' % debrid_service.lower(), '0'))
	return setting_value in (1, 2) if pack else setting_value == 1

def enabled_debrids_check(debrid_service):
	if not get_setting('NXTFlix.%s.enabled' % debrid_service) == 'true': return False
	return authorized_debrid_check(debrid_service)

def authorized_debrid_check(debrid_service):
	if get_setting('NXTFlix.%s.token' % debrid_service) in (None, ''): return False
	return True

def check_premium_account_status():
	return get_setting('NXTFlix.check_premium_account_status', 'false') == 'true'

def playback_settings():
	return (int(get_setting('NXTFlix.playback.watched_percent', '90')), int(get_setting('NXTFlix.playback.resume_percent', '5')))

def monitor_playback():
	return get_setting('NXTFlix.playback.monitor_success', 'true') == 'true'

def playback_attempt_pause():
	return int(get_setting('NXTFlix.playback.pause_start', '1500'))

def limit_resolve():
	return get_setting('NXTFlix.playback.limit_resolve', 'false') == 'true'

def disable_content_lookup():
	return get_setting('NXTFlix.playback.disable_content_lookup') == 'true'

def easynews_max_retries():
	return int(get_setting('NXTFlix.playback.easynews_max_retries', '1'))

def show_unaired_watchlist():
	return get_setting('NXTFlix.show_unaired_watchlist', 'false') == 'true'
	
def movies_directory():
	return translate_path(get_setting('NXTFlix.movies_directory'))
	
def tv_show_directory():
	return translate_path(get_setting('NXTFlix.tv_shows_directory'))
...
def source_folders_directory(media_type, source):
	setting = 'NXTFlix.%s.movies_directory' % source if media_type == 'movie' else 'NXTFlix.%s.tv_shows_directory' % source
	if get_setting(setting) not in ('', 'None', None): return translate_path( get_setting(setting))
	else: return False

def avoid_episode_spoilers():
	return get_setting('NXTFlix.avoid_episode_spoilers', 'false') == 'true'

def paginate(is_home):
	paginate_lists = int(get_setting('NXTFlix.paginate.lists', '0'))
...
def jump_to_enabled():
	return int(get_setting('NXTFlix.paginate.jump_to', '0'))

def use_year_in_search():
	return get_setting('NXTFlix.meta_use_year_in_search') == 'true'

def quality_filter(setting):
	return get_setting('NXTFlix.%s' % setting).split(', ')

def audio_filters():
	setting = get_setting('NXTFlix.filter_audio')
	if setting in ('', None): return []
	return setting.split(', ')

def include_prerelease_results():
	return get_setting('NXTFlix.include_prerelease_results', 'true') == 'true'

def auto_play(media_type):
	return get_setting('NXTFlix.auto_play_%s' % media_type, 'false') == 'true'

def autoplay_next_episode():
	if auto_play('episode') and get_setting('NXTFlix.autoplay_next_episode', 'false') == 'true': return True
	else: return False

def auto_rescrape_with_all():
	return get_setting('NXTFlix.results.autorescrape_with_all', 'false') == 'true'

def auto_nextep_settings():
	window_percentage = 100 - int(get_setting('NXTFlix.autoplay_next_window_percentage', '95'))
	use_chapters = get_setting('NXTFlix.autoplay_use_chapters', 'true') == 'true'
	scraper_time = int(get_setting('NXTFlix.results.timeout', '60')) + 20
	alert_method = int(get_setting('NXTFlix.autoplay_alert_method', '0'))
	default_action = default_action_dict[get_setting('NXTFlix.autoplay_default_action', '1')] if alert_method == 0 else 'cancel'
...
def filter_status(filter_type):
	return int(get_setting('NXTFlix.filter_%s' % filter_type, '0'))

def ignore_results_filter():
	return int(get_setting('NXTFlix.results.ignore_filter', '0'))
...
def auto_start_NXTFlix():
	return get_setting('NXTFlix.auto_start_NXTFlix', 'false') == 'true'

def furk_active():
	if get_setting('NXTFlix.provider.furk', 'false') == 'true':
		if not get_setting('NXTFlix.furk_api_key'):
			furk_status = False if '' in (get_setting('NXTFlix.furk_login'), get_setting('NXTFlix.furk_password')) else True
		else: furk_status = True
	else: furk_status = False
	return furk_status

def easynews_active():
	if get_setting('NXTFlix.provider.easynews', 'false') == 'true':
		easynews_status = False if '' in (get_setting('NXTFlix.easynews_user'), get_setting('NXTFlix.easynews_password')) else True
	else: easynews_status = False
	return easynews_status

def tv_progress_location():
	return int(get_setting('NXTFlix.tv_progress_location', '0'))

def extras_enable_extra_ratings():
	return get_setting('NXTFlix.extras.enable_extra_ratings', 'true') == 'true'

def extras_enabled_ratings():
	return get_setting('NXTFlix.extras.enabled_ratings', 'Meta, Tom/Critic, Tom/User, IMDb, TMDb').split(', ')

def extras_enable_scrollbars():
	return get_setting('NXTFlix.extras.enable_scrollbars', 'true')

def extras_exclude_non_acting():
	return get_setting('NXTFlix.extras.exclude_non_acting_roles', 'true') == 'true'

def extras_windowed_playback():
	return get_setting('NXTFlix.extras.windowed_playback', 'false') == 'true'

def extras_enabled_menus():
	setting = get_setting('NXTFlix.extras.enabled', '2000,2050,2051,2052,2053,2054,2055,2056,2057,2058,2059,2060,2061,2062,2063')
	if setting in ('', None, 'noop', []): return []
	return [int(i) for i in setting.split(',')]

def check_prescrape_sources(scraper, media_type):
	if scraper in prescrape_scrapers_tuple: return get_setting('NXTFlix.check.%s' % scraper) == 'true'
	if get_setting('NXTFlix.check.%s' % scraper) == 'true' and auto_play(media_type): return True
	else: return False

def external_scraper_info():
	module = get_setting('NXTFlix.external_scraper.module', None)
	if module: return module, module.split('.')[-1]
	else: return None, ''

def filter_by_name(scraper):
	if get_property('fs_filterless_search') == 'true': return False
	return get_setting('NXTFlix.%s.title_filter' % scraper, 'false') == 'true'

def easynews_language_filter():
	enabled = get_setting('NXTFlix.easynews.filter_lang') == 'true'
	if enabled: filters = get_setting('NXTFlix.easynews.lang_filters').split(', ')
	else: filters = []
	return enabled, filters

def results_sort_order():
	sort_direction = -1 if get_setting('NXTFlix.results.size_sort_direction') == '0' else 1
	return (
			...
			)[int(get_setting('NXTFlix.results.sort_order', '1'))]
...
def active_internal_scrapers():
	...
	active = [i.split('.')[1] for i in settings if get_setting('NXTFlix.%s' % i) == 'true']
	return active

def provider_sort_ranks():
	fu_priority = int(get_setting('NXTFlix.fu.priority', '6'))
	en_priority = int(get_setting('NXTFlix.en.priority', '7'))
	rd_priority = int(get_setting('NXTFlix.rd.priority', '8'))
	ad_priority = int(get_setting('NXTFlix.ad.priority', '9'))
	pm_priority = int(get_setting('NXTFlix.pm.priority', '10'))
...
def auto_resume(media_type):
	auto_resume = get_setting('NXTFlix.auto_resume_%s' % media_type)
	if auto_resume == '1': return True
	if auto_resume == '2' and auto_play(media_type): return True
	else: return False

def scraping_settings():
	highlight_type = int(get_setting('NXTFlix.highlight.type', '0'))
	if highlight_type == 3:
		highlight = get_setting('NXTFlix.scraper_single_highlight', 'FF008EB2')
	...
	if highlight_type in (0, 1):
		furk_highlight = get_setting('NXTFlix.provider.furk_colour', 'FFE6002E')
		easynews_highlight = get_setting('NXTFlix.provider.easynews_colour', 'FF00B3B2')
		debrid_cloud_highlight = get_setting('NXTFlix.provider.debrid_cloud_colour', 'FF7A01CC')
		folders_highlight = get_setting('NXTFlix.provider.folders_colour', 'FFB36B00')
		if highlight_type == 0:
			hoster_highlight = get_setting('NXTFlix.hoster.identify', 'FF0166FF')
			torrent_highlight = get_setting('NXTFlix.torrent.identify', 'FFFF00FE')
		else:
			rd_highlight = get_setting('NXTFlix.provider.rd_colour', 'FF3C9900')
			pm_highlight = get_setting('NXTFlix.provider.pm_colour', 'FFFF3300')
			ad_highlight = get_setting('NXTFlix.provider.ad_colour', 'FFE6B800')
	elif highlight_type == 2:
		highlight_4K = get_setting('NXTFlix.scraper_4k_highlight', 'FFFF00FE')
		highlight_1080P = get_setting('NXTFlix.scraper_1080p_highlight', 'FFE6B800')
		highlight_720P = get_setting('NXTFlix.scraper_720p_highlight', 'FF3C9900')
		highlight_SD = get_setting('NXTFlix.scraper_SD_highlight', 'FF0166FF')
...
def omdb_api_key():
	return get_setting('NXTFlix.omdb_api', '')
...
def default_all_episodes():
	return int(get_setting('NXTFlix.default_all_episodes'))

def max_threads():
	if not get_setting('NXTFlix.limit_concurrent_threads', 'false') == 'true': return 60
	return int(get_setting('NXTFlix.max_threads', '60'))

def widget_hide_next_page():
	return get_setting('NXTFlix.widget_hide_next_page', 'false') == 'true'

def show_unaired():
	return get_setting('NXTFlix.show_unaired', 'true') == 'true'

def use_season_title():
	return get_setting('NXTFlix.use_season_title', 'true') == 'true'
...
def date_offset():
	return int(get_setting('NXTFlix.datetime.offset', '0')) + 5

def single_ep_display_title():
	return int(get_setting('NXTFlix.single_ep_display', '0'))

def single_ep_format():
	return single_ep_format_dict[get_setting('NXTFlix.single_ep_format', '1')]

def include_year_in_title(media_type):
	return int(get_setting('NXTFlix.include_year_in_title', '0')) in year_in_title_dict[media_type]

def extras_open_action(media_type):
	return int(get_setting('NXTFlix.extras.open_action', '0')) in extras_open_action_dict[media_type]

def tmdb_api_key():
	return get_setting('NXTFlix.tmdb_api', tmdb_default_api)

def get_fanart_data():
	return get_setting('NXTFlix.get_fanart_data') == 'true'

def get_resolution():
	return resolution_tuple[int(get_setting('NXTFlix.image_resolutions', '2'))]

def get_language():
	return get_setting('NXTFlix.meta_language', 'en')

def get_meta_filter():
	return get_setting('NXTFlix.meta_filter', 'true')

def get_mpaa_region():
	return get_setting('NXTFlix.meta_mpaa_region', 'US')

def widget_hide_watched():
	return get_setting('NXTFlix.widget_hide_watched', 'false') == 'true'
...
def fanarttv_default():
	return get_setting('NXTFlix.fanarttv.default', 'false') == 'true'

def nextep_display_settings():
	include_airdate = get_setting('NXTFlix.nextep.include_airdate', 'false') == 'true'
	return {'unaired_color': 'red', 'unwatched_color': 'darkgoldenrod', 'include_airdate': include_airdate}

def nextep_content_settings():
	sort_type = int(get_setting('NXTFlix.nextep.sort_type', '0'))
	sort_order = int(get_setting('NXTFlix.nextep.sort_order', '0'))
	sort_direction = sort_order == 0
	sort_key = 'last_played' if sort_type == 0 else 'first_aired' if sort_type == 1 else 'name'
	include_unaired = get_setting('NXTFlix.nextep.include_unaired', 'false') == 'true'
	include_unwatched = int(get_setting('NXTFlix.nextep.include_unwatched', '0'))
	sort_airing_today_to_top = get_setting('NXTFlix.nextep.sort_airing_today_to_top', 'false') == 'true'
...
def update_delay():
	return int(get_setting('NXTFlix.update.delay', '45'))

def update_action():
	return int(get_setting('NXTFlix.update.action', '2'))