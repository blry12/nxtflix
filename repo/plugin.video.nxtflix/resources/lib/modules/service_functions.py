...
class FirstRunActions:
	def run(self):
		...
		if get_setting('first_use', 'false') == 'true':
			lines = ('By default, NXTFlix has [B]Local Folders[/B], [B]Debrid Cloud[/B], [B]Furk[/B] and [B]Easynews[/B] scrapers.',
			'These scrapers are designed for use on media you have purchased and organized through these services.',
			'NXTFlix is [B]unable[/B] to search online for sources outside of these services.',
			'If you wish to use an external scraper package within NXTFlix, you must select a module at[CR][B]NXTFlix Settings->Providers->Choose External Scrapers Module.[/B]',
			'The use of external scrapers is outside of the intended use of NXTFlix, and will not be supported by NXTFlix.')
...
class PremiumExpiryCheck:
	def run(self):
		...
		try:
			self.run_check()
			if self.make_message(): ok_dialog('NXTFlix - Premium Accounts Expiring...', '\n'.join(self.message))
	...
	def run(self, sender, method, data):
		if sender == 'xbmc':
			if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
				...
				logger('OnNotificationActions', 'PAUSING NXTFlix Services Due to Device Sleep')
			elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
				clear_property(pause_services_prop)
				logger('OnNotificationActions', 'UNPAUSING NXTFlix Services Due to Device Awake')
