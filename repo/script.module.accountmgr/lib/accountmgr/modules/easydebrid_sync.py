    ...
    #NXTFlix Light
        try:
                if xbmcvfs.exists(var.chk_nxtflixlt) and xbmcvfs.exists(var.chkset_nxtflixlt): #Check that the addon is installed and settings.db exists
                    ...
                    from accountmgr.modules import easydebrid_db
                    conn = easydebrid_db.create_conn(var.nxtflixlt_settings_db)
                    ...
                    with conn:
                        ...
                        chk_auth_nxtflixlt = str(auth_ed)
                        cursor.close()
                        ...
                        if not str(var.chk_accountmgr_ed) == chk_auth_nxtflixlt: #Compare Account Mananger data to Add-on data. If they match, authorization is skipped
                            
                            #Write settings to database
                            from accountmgr.modules import easydebrid_db
                            easydebrid_db.auth_nxtflixlt_ed()
                            var.remake_settings()
        except:
                xbmc.log('%s: NXTFlix Light Easy Debrid Failed!' % var.amgr, xbmc.LOGINFO)
                pass
