...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: set_property('nxtflixlight.exit_params', folder_path())
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			set_properties({'nxtflixlight.extras_params': extras_params, 'nxtflixlight.options_params': options_params,
							'belongs_to_collection': belongs_to_movieset, 'nxtflixlight.more_like_this_params': more_like_this_params})
	...
	def worker(self):
		...
		self.watched_title = 'Trakt' if self.watched_indicators == 1 else 'NXTFlix Light'
