...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'NXTFlix Light')
	...
	def tools(self):
		...
		if get_setting('nxtflixlight.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('nxtflixlight.use_viewtypes', 'true') == 'true' and not get_setting('nxtflixlight.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		nxtflixlight_clogpath = tp('special://home/addons/plugin.video.nxtflixlight/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': nxtflixlight_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def exit_media_menu(self):
		params = get_property('nxtflixlight.exit_params')
		if params: return container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.nxtflixlight/resources/text/tips'
