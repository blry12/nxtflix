...
def build_episode_list(params):
	def _process():
		for item in episodes_data:
			try:
				...
				set_properties({'nxtflixlight.extras_params': extras_params, 'nxtflixlight.options_params': options_params, 'episode_type': episode_type})
	...
	watched_title = 'Trakt' if watched_indicators == 1 else 'NXTFlix Light'
...
def build_single_episode(list_type, params={}):
	...
	if list_type == 'episode.next':
		...
		else: resformat, resinsert, list_type = '%Y-%m-%d %H:%M:%S', '2000-01-01 00:00:00', 'episode.next_nxtflixlight'
