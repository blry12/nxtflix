...
destination_check = translate_path('special://home/addons/plugin.video.nxtflixlight/')
changelog_location = translate_path('special://home/addons/plugin.video.nxtflixlight/resources/text/changelog.txt')
downloads_icon = get_icon('downloads')
addon_dir = 'plugin.video.nxtflixlight'
zipfile_name = 'plugin.video.nxtflixlight-%s.zip'
heading_str = 'NXTFlix Light Updater'
error_str = 'Error'
notification_occuring_str = 'NXTFlix Light Update Occuring'
notification_available_str = 'NXTFlix Light Update Available'
notification_updating_str = 'NXTFlix Light Performing Update'
notification_rollback_str = 'NXTFlix Light Performing Rollback'
...
success_str = '[CR]Success.[CR]NXTFlix Light updated to version [B]%s[/B]'
rollback_heading_str = 'Choose Rollback Version'
success_rollback_str = '[CR]Success.[CR]NXTFlix Light rolled back to version [B]%s[/B]'
confirm_rollback_str = 'Are you sure?[CR]Version [B]%s[/B] will overwrite your current installed version.' \
						'[CR]NXTFlix Light will set your update action to [B]OFF[/B] if rollback is successful'
...
no_changes_str = 'You are running the current version of NXTFlix Light.[CR][CR]There is no new version changelog to view.'

def get_location(insert=''):
	return 'https://github.com/%s/%s/raw/main/packages/%s' % (get_setting('nxtflixlight.update.username'), get_setting('update.location'), insert)

def get_versions():
	try:
		result = requests.get(get_location('nxtflix_light_version'))
...
def get_changes(online_version=None):
	try:
		...
		show_busy_dialog()
		result = requests.get(get_location('nxtflix_light_changes'))
		hide_busy_dialog()
...
def rollback_check():
	...
	url = 'https://api.github.com/repos/%s/%s/contents/packages' % ('blry12', 'blry12.github.io')#(get_setting('update.username'), get_setting('update.location'))
	show_busy_dialog()
	results = requests.get(url)
	hide_busy_dialog()
	if results.status_code != 200: return ok_dialog(heading=heading_str, text=error_rollback_str)
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results.json() if 'plugin.video.nxtflixlight' in i['name'] \
