...
tmdb_default_api = '76067629fab243b989a68881eb1f63ef'
_id = '793fda23d5ab3f352dc5856e5aa3a43c150402406cadf81a419bce23fab15e46'
_secret = '2cc8aaac698563a9ad5d3be4cceb2a02543fc12b600c191d37565acfb2b5fdc2'
...
single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_nxtflixlight', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.nxtflixlight').getAddonInfo(info)

def addon_version():
	return get_property('nxtflixlight.addon_version') or addon_info('version')

def addon_path():
	return get_property('nxtflixlight.addon_path') or addon_info('path')

def addon_profile():
	return get_property('nxtflixlight.addon_profile') or translatePath(addon_info('profile'))

def addon_icon():
	return get_property('nxtflixlight.addon_icon') or addon_info('icon')

def addon_fanart():
	return get_property('nxtflixlight.addon_fanart') or addon_info('fanart')

def get_icon(image_name):
	return img_url % getattr(icons, image_name, 'I1JJhji')

def get_addon_fanart():
	return get_property('nxtflixlight.default_addon_fanart') or addon_fanart()

def build_url(url_params):
	return 'plugin://plugin.video.nxtflixlight/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('nxtflixlight.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('nxtflixlight.%s' % view_type) or None
...
def addon(addon_id='plugin.video.nxtflixlight'):
...
def run_addon(addon='plugin.video.nxtflixlight', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'nxtflixlight' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.nxtflixlight'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.nxtflixlight'):
	import time
...
def external_scraper_settings():
	try:
		external = get_property('nxtflixlight.external_scraper.module')
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('NXTFlix Light', line1, icon or addon_icon(), time)
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('nxtflixlight.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('nxtflixlight.playback.volumecheck_percent') or '50'), 100)
...
def toggle_language_invoker():
	...
	close_all_dialog()
	addon_xml = translate_path('special://home/addons/plugin.video.nxtflixlight/addon.xml')
...
def upload_logfile(params):
	...
	try:
		...
		UserAgent = 'nxtflixlight %s' % addon_version()
