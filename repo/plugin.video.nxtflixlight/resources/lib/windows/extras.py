...
setting_base, label_base, ratings_icon_base = 'nxtflixlight.extras.%s.button', 'button%s.label', 'nxtflixlight_flags/ratings/%s'
separator = '[B]  •  [/B]'
	...
	def assign_buttons(self):
		...
		for item in button_ids[:-1]:
			...
			try:
				...
				self.restore_setting_default({'setting_id': setting_id.replace('nxtflixlight.', ''), 'silent': 'true'})
				button_action = self.get_setting(setting_id)
				button_label = extras_button_label_values[self.media_type][button_action]
