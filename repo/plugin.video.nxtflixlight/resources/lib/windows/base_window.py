...
current_skin_prop, current_font_prop = 'nxtflixlight.current_skin', 'nxtflixlight.current_font'
addon_skins_folder = 'special://home/addons/plugin.video.nxtflixlight/resources/skins/Default/1080i/'
...
def window_manager(obj):
	def close():
		obj.close()
		clear_property('nxtflixlight.window_loaded')
		clear_property('nxtflixlight.window_stack')

	def monitor():
		timer = 0
		while not get_property('nxtflixlight.window_loaded') == 'true' and timer <= 5:
			sleep(50)
			timer += 0.05
		hide_busy_dialog()
		obj.close()
	...
	def get_stack():
		try: window_stack = json.loads(get_property('nxtflixlight.window_stack'))
		except: window_stack = []
		return window_stack

	def add_to_stack(params):
		window_stack.append(params)
		set_property('nxtflixlight.window_stack', json.dumps(window_stack))
...
class BaseDialog(window_xml_dialog):
	...
	def set_home_property(self, prop, value):
		set_property('nxtflixlight.%s' % prop, value)

	def get_home_property(self, prop):
		return get_property('nxtflixlight.%s' % prop)

	def clear_home_property(self, prop):
		return clear_property('nxtflixlight.%s' % prop)
...
class FontUtils:
	...
	def match_font(self, size, bold, fallback):
		font_tag = 'nxtflixlight_%s%s' % (size, '_BOLD' if bold else '')
