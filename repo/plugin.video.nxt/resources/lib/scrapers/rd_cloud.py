...
class source:
	...
	def results(self, info):
		try:
			...
			from modules.kodi_utils import logger
			logger('nxt real-debrid scraper Exception', str(e))
		internal_results(self.scrape_provider, self.sources)
		return self.sources

	def _gather_assigned_content(self):
		pre_assigned_content = gather_assigned_content("'nxt_RD_%'")
	...
	def _scrape_cloud(self):
		try:
			...
			for item in my_cloud_files:
				...
				if test_assigned_content('nxt_RD_%s' % item['id'], self.pre_assigned_content): results_append((item['id'], True))
