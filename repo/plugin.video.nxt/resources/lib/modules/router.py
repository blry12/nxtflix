...
def sys_exit_check():
	if get_setting('nxt.reuse_language_invoker') == 'true' and external(): return True
	return False
...
def routing(sys):
	...
	if 'playback.' in mode:
		...
		if mode == 'playback.video':
			...
			return nxtPlayer().run(_get('url', None), _get('obj', None))
	if 'choice' in mode:
	...
	if mode == 'open_settings':
		...
		return open_settings(_get('query', '0.0'), _get('addon', 'plugin.video.nxt'))
