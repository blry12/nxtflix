	...
	def media_watched_marker(self, force_watched=False):
		...
		try:
			if self.current_point >= self.set_watched or force_watched:
				...
				clear_property('nxt.random_episode_history')
...
class Subtitles(xbmc_player):
	def __init__(self):
		...
		self.auto_enable = get_setting('nxt.subtitles.auto_enable')
		self.subs_action = get_setting('nxt.subtitles.subs_action')
		self.language = get_setting('nxt.subtitles.language_primary')
	...
	def get(self, query, imdb_id, season, episode, secondary_search=False):
		...
		sub_filename = 'nxtSubs_%s_%s_%s' % (imdb_id, season, episode) if season else 'nxtSubs_%s' % imdb_id
		...
		secondary_language = get_setting('nxt.subtitles.language_secondary')
