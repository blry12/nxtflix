...
addon_object = xbmcaddon.Addon('plugin.video.nxt')
...
addon_icon = path_join(addon_path, 'resources', 'media', 'nxt_icon.png')
addon_fanart = addon_object.getSetting('addon_fanart') or path_join(addon_path, 'resources', 'media', 'nxt_fanart.png')
...
int_window_prop, pause_services_prop, suppress_sett_dict_prop, highlight_prop = 'nxt.internal_results.%s', 'nxt.pause_services', 'nxt.suppress_settings_dict', 'nxt.main_highlight'
custom_context_main_menu_prop, custom_context_prop, sett_addoninfo_active_prop = 'nxt.custom_context_main_menu', 'nxt.custom_context_menu', 'nxt.setting_addoninfo_active'
pause_settings_prop, use_skin_fonts_prop, custom_info_prop = 'nxt.pause_settings', 'nxt.use_skin_fonts', 'nxt.custom_info_dialog'
current_skin_prop, current_font_prop = 'nxt.current_skin', 'nxt.current_font'
...
def build_url(url_params):
	return 'plugin://plugin.video.nxt/?%s' % urlencode(url_params)
...
def end_directory(handle, cacheToDisc=None):
	if cacheToDisc == None: cacheToDisc = get_setting('nxt.kodi_menu_cache') == 'true'
...
def addon(addon_id='plugin.video.nxt'):
...
def run_addon(addon='plugin.video.nxt', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'nxt' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.nxt'):
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.nxt'):
	import time
	import sqlite3 as database
	try:
...
def set_view(view_type):
	...
	set_property('nxt.%s' % view_type, view_id)
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_setting('nxt.playback.volumecheck_enabled', 'false') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_setting('nxt.playback.volumecheck_percent', '50')), 100)
...
def toggle_language_invoker():
	...
	current_addon_setting = get_setting('nxt.reuse_language_invoker', 'true')
...
def upload_logfile(params):
	...
	try:
		...
		UserAgent = 'nxt %s' % addon_object.getAddonInfo('version')
...
def open_settings(query, addon='plugin.video.nxt'):
	hide_busy_dialog()
	if query:
		try:
...
def external_scraper_settings():
	try:
		external = get_setting('nxt.external_scraper.module', None)
...
def get_setting(setting_id, fallback=''):
	return get_property(setting_id) or addon_object.getSetting(setting_id.replace('nxt.', '')) or fallback
...
def make_settings_props():
	...
	try:
		...
		for item in all_settings(): set_property('nxt.%s' % item[0], item[1])
