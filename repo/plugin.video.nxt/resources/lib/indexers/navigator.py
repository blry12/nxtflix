...
log_utils_str, tips_use_str, views_str, updates_str, nxt_str, all_str, cache_str, clean_str = ls(32777), ls(32518), ls(32510), ls(32196), ls(32036), ls(32129), ls(32524), ls(32526)
...
class Navigator:
	...
	def tools(self):
		self.add({'mode': 'open_settings', 'isFolder': 'false'}, settings_ins % nxt_str, 'settings')
		if get_setting('nxt.external_scraper.module', None):
	...
	def accounts_manager(self):
		trakt_active = get_setting('nxt.trakt.user') != ''
	...
	def changelog_utils(self):
		nxt_clogpath = tp(log_path % 'plugin.video.nxt/resources/text')
		mh_str = '[B]%s[/B]: %s  [I](v.%s)[/I]' % (changelog_str.upper(), nxt_str, addon().getAddonInfo('version'))
		self.add({'mode': 'show_text', 'heading': mh_str, 'file': nxt_clogpath, 'font_size': 'large', 'isFolder': 'false'}, mh_str, 'lists', False)
	...
	def folder_navigator(self):
		...
		def _process():
			for info in results:
				try:
					...
					linked_folder = test_assigned_content('nxt_FOLDERS_%s' % link_id, assigned_content)
					...
					info_tag.setPlot(' ')
					listitem.setProperty('nxt.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_title, 'iconImage': folder_icon,
		...
		assigned_content = gather_assigned_content("'nxt_FOLDERS_%s" % setting_id + "_%'")
		...
		def _builder():
			for item in folder_info:
				for media_type in ('movie', 'tvshow'):
					...
					display_name = get_setting('nxt.%s.display_name' % setting_id)
					if action == 'browse':
		...
		def _builder():
			for i in folders:
				try:
					...
					info_tag = listitem.getVideoInfoTag()
					info_tag.setPlot(' ')
					listitem.setProperty('nxt.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': folder_icon}))
					if self.is_home: listitem.setProperty('nxt.external', 'true')
		...
		def _process():
			for item_position, item in enumerate(contents):
				try:
					...
					info_tag.setPlot(' ')
					listitem.addContextMenuItems(cm)
					listitem.setProperty('nxt.context_main_menu_params', menu_editor_url)
	...
	def exit_media_menu(self):
		params = get_property('nxt.exit_params')
		if params: return container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.nxt/resources/text/tips'
	...
	def add(self, url_params, list_name, iconImage='folder', contextmenu_edit=True, original_image=False):
		...
		if contextmenu_edit:
			...
			listitem.addContextMenuItems(cm)
			listitem.setProperty('nxt.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': list_name, 'iconImage': iconImage}))
			if self.is_home: listitem.setProperty('nxt.external', 'true')
