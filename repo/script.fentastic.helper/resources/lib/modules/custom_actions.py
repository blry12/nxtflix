...
def copy_colors():
    try:
        ...
        if not xbmcvfs.exists(src):
                xbmc.log(f'FENtastic: Optional color file not found -> {src}',xbmc.LOGERROR)
                return False
...
def copy_fentastic_defaults(mode='fen'):
    try:
        ...
        sources = {'fen': os.path.join(base_path,'resources', 'default_colors', 'fentastic', 'defaults.xml'),'fenplus': os.path.join(base_path,'resources', 'default_colors', 'fentastic_plus', 'defaults.xml')}
        dst = os.path.join(base_path,'colors', 'defaults.xml')
        src = sources.get(mode)

        if not src or not xbmcvfs.exists(src):
                xbmc.log(f'FENtastic: Defaults file not found for mode "{mode}"',xbmc.LOGERROR)
                return False
...
def choose_lookfeel():
    ...
    if choice == 0:  # Classic Custom Artwork
        ...
        xbmc.executebuiltin('Skin.Reset(CurrentSkinColor)')
